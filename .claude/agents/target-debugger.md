...
| esp-target-debug   | PRIMARY playbook for Espressif boards (user-level from agentrc) — built-in USB-Serial-JTAG attach, the PHY map that decides whether JTAG exists, FreeRTOS threads via ESP_RTOS; target-debug still supplies the methodology     |
