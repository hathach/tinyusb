...
Examples must be built for the target board(s) — see [Build and Validate](../../../CLAUDE.md#build-and-validate). For a **local** run of a board without a `"variant"` list in the config, the `build` skill's `--shared` produces `cmake-build/cmake-build-<board>/`, the folder `hil_test.py` flashes from by default. A **remote** run stages the same folder; see Remote execution below.

A board with a `"variant"` list runs each variant from `cmake-build-<variant>/`, built with that variant's `flags` and `defines`; `--shared` never reads the roster, so a bare `--shared` applies neither and cannot build a variant not named after the board. On a local run an unbuilt variant's tests are reported `Skip (no binary)` and add no failure to the exit status (`stm32f723disco-DMA` goes untested), except on a one-test run (`-t device/usbtest`, a single-test `-bt`): there a later unbuilt variant fails as `same-PID boundary ... not cleared (no board_test binary)`, a non-zero exit that names `board_test`, not the missing variant build. A self-named variant runs whatever `cmake-build-<board>/` holds, without its flags if a bare `--shared` built it (`raspberry_pi_pico`'s `CFG_TUH_RPI_PIO_USB=1`). Build such a board with `hil_test.py --build` instead of `--shared` on a local run. For a remote run, `python3 .github/scripts/hil_ci_set_matrix.py <config>` prints the `tools/build.py` arguments CI builds each variant with, as JSON arrays keyed by toolchain: run `python3 tools/build.py <entry>` for every entry that starts with `-b <board> ` or equals `-b <board>`. Neither route drops flags or defines cached by an earlier configure, so remove `cmake-build/cmake-build-<variant>/` first if something else configured it differently. (This applies to `hil_test.py`; `hil_pool_check.py` builds its own missing firmware.)
