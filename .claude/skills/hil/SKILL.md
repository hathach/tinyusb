...
A board with a `"variant"` list runs each variant from `cmake-build-<variant>/`, built with that variant's `flags` and `defines`; `--shared` builds only `cmake-build-<board>/`, with neither. On a local run an unbuilt variant is reported `Skip (no binary)` and adds no failure to the exit status (`stm32f723disco-DMA` goes untested), and a self-named variant runs whatever `cmake-build-<board>/` holds, without its flags if `--shared` built it (`raspberry_pi_pico`'s `CFG_TUH_RPI_PIO_USB=1`). Build such a board with `hil_test.py --build` instead of `--shared` on a local run. For a remote run, `python3 .github/scripts/hil_ci_set_matrix.py <config>` prints the `tools/build.py` arguments CI builds each variant with, as JSON arrays keyed by toolchain: run `python3 tools/build.py <entry>` for every entry that starts with `-b <board> ` or equals `-b <board>`. (This applies to `hil_test.py`; `hil_pool_check.py` builds its own missing firmware.)
