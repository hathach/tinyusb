...
- **The gap is a hard `parser.error`, not a silent limitation.**
  In `tools/metrics_compare_base.py`'s argument checks:
  ```python
  ...
  ```
  and `--ci` unconditionally sets `args.combined = True`, so today `--ci` **requires**
...
- **The linkermap combine path is the model to match the *output* of, not necessarily the
  implementation**: `metrics_compare_base.py`'s combined block builds a `_combined` dir, concatenates
