...
Consequence of keeping the matrix shape: CircleCI's generated `config2.yml` does not inflate
to one entry per family. `hil-build`
...
check, and HIL is unaffected because `hil-build` is a separate matrix.
