...
```json
{
  "boards": [
    ...
    { "board": "raspberry_pi_pico", "note": "optional" }
  ],
  "uncovered": {
...
- One entry per pinned board. The drivers it covers are derived from its
  `hw/bsp/family.json` row: `cmake.portable` filtered by `cmake.roles`
  (`dcd_*` with device or dual, `hcd_*`/`ehci`/`ohci` with host or dual).
  Driver names are source basenames from `src/portable/**`; `template` is
  excluded.
...
**Coverage checker** `tools/drivers_coverage_check.py`, wired as a
pre-commit hook (runs locally and in the pre-commit CI lane). Fails when:

- a driver file exists that no pinned board covers and `uncovered` does not
  waive (with a non-empty reason); a waiver may overlap derived coverage;
- an `uncovered` name matches no real file (catches renames);
- a board does not exist under `hw/bsp`, or has no `family.json` cmake row.
