...
{
  /* Derive a stable, locally-administered MAC from the F4 96-bit unique
   * device ID. Bit 1 of the first byte is set (locally administered),
   * bit 0 is cleared (unicast). */
  static uint8_t macaddress[6];
  {
    const uint32_t *uid = (const uint32_t *)UID_BASE;  /* 0x1FFF7A10 on F4 */
    uint32_t a = uid[0] ^ uid[2];
    uint32_t b = uid[1];
    macaddress[0] = 0x02;
    macaddress[1] = (uint8_t)(a >> 0);
    macaddress[2] = (uint8_t)(a >> 8);
    macaddress[3] = (uint8_t)(a >> 16);
    macaddress[4] = (uint8_t)(b >> 0);
    macaddress[5] = (uint8_t)(b >> 8);
  }
...
/*
 * HAL_ETH_MspInit (RMII pin map, peripheral clocks) lives in the
 * board's BSP at hw/bsp/<family>/boards/<board>/board_eth.c so the
 * example stays board-agnostic. HAL_ETH_Init() invokes the weak
 * HAL_ETH_MspInit symbol automatically; the linker pulls the BSP's
 * definition when family_add_eth() is called from the example's
 * CMakeLists.
 */
