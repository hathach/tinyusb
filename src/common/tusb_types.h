...
#define TUD_EPBUF_DCACHE_SIZE(_size)                                                                             \
  (CFG_TUD_MEM_DCACHE_ENABLE ? (TU_DIV_CEIL(_size, CFG_TUD_MEM_DCACHE_LINE_SIZE) * CFG_TUD_MEM_DCACHE_LINE_SIZE) \
                             : (_size))
...
#define TUH_EPBUF_DCACHE_SIZE(_size)                                                                             \
  (CFG_TUH_MEM_DCACHE_ENABLE ? (TU_DIV_CEIL(_size, CFG_TUH_MEM_DCACHE_LINE_SIZE) * CFG_TUH_MEM_DCACHE_LINE_SIZE) \
                             : (_size))
...
typedef struct TU_ATTR_PACKED {
  ...
  uint8_t  bMaxPacketSize0; ///< Maximum packet size for endpoint zero (only 8, 16, 32, or 64 are valid). For HS devices
                            ///< is fixed to 64.
...
typedef struct TU_ATTR_PACKED {
  uint8_t  bLength;         ///< Size of this descriptor in bytes
  uint8_t  bDescriptorType; ///< CONFIGURATION Descriptor Type
  uint16_t wTotalLength;  ///< Total length of data returned for this configuration. Includes the combined length of all
                          ///< descriptors (configuration, interface, endpoint, and class- or vendor-specific) returned
  ...
  uint8_t bmAttributes; ///< Configuration characteristics \n D7: Reserved (set to one)\n D6: Self-powered \n D5: Remote
                        ...
                        ///< source reports a non-zero value in bMaxPower to indicate the amount of bus power required
                        ///< and sets D6. The actual power source at runtime may be determined using the
                        ///< GetStatus(DEVICE) request (see USB 2.0 spec Section 9.4.5). \n If a device configuration
  ...
  uint8_t bMaxPower; ///< Maximum power consumption of the USB device from the bus in this specific configuration when
                     ///< the device is fully operational. Expressed in 2 mA units (i.e., 50 = 100 mA).
...
typedef struct TU_ATTR_PACKED {
  ...
  uint8_t bInterfaceNumber; ///< Number of this interface. Zero-based value identifying the index in the array of
                            ///< concurrent interfaces supported by this configuration.
  uint8_t
    bAlternateSetting; ///< Value used to select this alternate setting for the interface identified in the prior field
  uint8_t bNumEndpoints;   ///< Number of endpoints used by this interface (excluding endpoint zero). If this value is
                           ///< zero, this interface only uses the Default Control Pipe.
  uint8_t bInterfaceClass; ///< Class code (assigned by the USB-IF). \li A value of zero is reserved for future
                           ///< standardization. \li If this field is set to FFH, the interface class is
                           ///< vendor-specific. \li All other values are reserved for assignment by the USB-IF.
  uint8_t bInterfaceSubClass; ///< Subclass code (assigned by the USB-IF). \n These codes are qualified by the value of
                              ///< the bInterfaceClass field. \li If the bInterfaceClass field is reset to zero, this
                              ///< field must also be reset to zero. \li If the bInterfaceClass field is not set to FFH,
                              ///< all values are reserved for assignment by the USB-IF.
  uint8_t bInterfaceProtocol; ///< Protocol code (assigned by the USB). \n These codes are qualified by the value of the
                              ///< bInterfaceClass and the bInterfaceSubClass fields. If an interface supports
                              ///< class-specific requests, this code identifies the protocols that the device uses as
                              ///< defined by the specification of the device class. \li If this field is reset to zero,
                              ///< the device does not use a class-specific protocol on this interface. \li If this
                              ///< field is set to FFH, the device uses a vendor-specific protocol for this interface.
...
TU_ATTR_ALWAYS_INLINE static inline uint16_t tu_edpt_packet_size(const tusb_desc_endpoint_t *desc_ep) {
  return tu_le16toh(desc_ep->wMaxPacketSize) & 0x7FF;
}

#if CFG_TUSB_DEBUG
TU_ATTR_ALWAYS_INLINE static inline const char *tu_edpt_type_str(tusb_xfer_type_t t) {
  const tu_static char *str[] = {"control", "isochronous", "bulk", "interrupt"};
  return str[t];
}
#endif
