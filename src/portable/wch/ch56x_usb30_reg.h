...
// The register is a single one-shot TP mailbox shared by every endpoint: a second write that
// lands before the link layer has transmitted the first TP silently replaces it. Bidirectional
// traffic (NCM under iperf: IN and OUT arms back-to-back) lost OUT ERDYs this way and the
// flow-stopped host waited forever (bulk URBs stuck, zero errors). Hold a short drain window
// after the write - a TP is a handful of symbol times at 5 Gbps, so a few hundred core cycles
// cover link-layer queueing.
// The drain only serializes writers in the same context: an endpoint-completion ISR landing
// inside a task-context window could still replace that TP. Do NOT close this with an mstatus
// save/__disable_irq()/restore around the write+drain: on this core the mstatus RMW inside the
// (hardware-stack, naked-handler) interrupt scheme corrupts interrupt return state and the
// device stops answering SET_ADDRESS (hw-measured 2026-08-12, CDC builds dead 0/12). The
// residual task-vs-ISR window is accepted and has not been observed to lose a TP in practice.
TU_ATTR_ALWAYS_INLINE static inline void usbss_send_erdy(uint8_t ep, bool dir_in, uint8_t nump) {
  ...
  for (volatile uint32_t i = 0; i < 60; i++) {} // ~0.5 us at 120 MHz: let the TP leave the mailbox
}
